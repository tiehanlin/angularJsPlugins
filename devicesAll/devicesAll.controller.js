(function() {
	...
	angular
		...
		.run(function(c8yNavigator, c8yJsonSchemas, c8ySettings) {
			var option = {
				category: 'mylogo',
				key: 'mylogo'
			};
			c8ySettings.detailValue(option).then(function(value) {
				if (value !== false) {
					var logo = document.createElement('style')
					logo.innerHTML = '#app .navigator .title{background-image:url(' + value + ');}';
		...
		.controller('devicesAllCtrl', function($scope, $timeout, $q, c8yTitle, c8yAlarms, c8yDevices, c8yGroups, c8yPermissions, c8yUser) {
			...
			function alarm(type) {
				...
				c8yAlarms.list({
					...
					deferred.resolve(alarms.statistics.totalPages);
				});
				...
				$scope.lineOption = {
					toolbox: {
						...
						feature: {
							restore: {
								show: true,
								icon: 'image://appinterface_devicesAll/img/quanbushebei.png'
							}
						}
					},
					tooltip: {
			...
			function createMarker(result, img, maps, v) {
				$scope.markerOpts.push({
					...
					icon: {
						image: 'appinterface_devicesAll/img/' + img + '.png',
						size: {
							width: 32,
							height: 32
						}
			...
			function del(img) {
				$scope.$apply(function() {
					for (var i = 0; i < mars.length; i++) {
						if (mars[i].G.icon.G.image == "appinterface_devicesAll/img/" + img + ".png") {
							mars[i].show()
