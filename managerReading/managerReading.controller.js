(function() {
	...
	angular
		...
		.controller('ManagerModalInstanceCtrl', [
			...
			// 'items',
			'c8yInventory',
			ManagerModalInstanceCtrl
		]);

		function managerReadingCtrl (
			$scope,
			$routeParams,
			c8yDeviceControl,
			c8yInventory,
			...
			vm.modal = {};
			c8yInventory.childAssets(vm.groupId).then(function(res){
				console.log(res);
				vm.devices = res
			})
      ...
      // })
          ...
          var modalInstance = $uibModal.open({
              ...
              templateUrl: 'managerModalContent.html',
              controller: 'ManagerModalInstanceCtrl',
              controllerAs: 'vm',
              size: size,
              appendTo: parentElem,
    ...
    function ManagerModalInstanceCtrl($scope,$routeParams,$uibModalInstance, c8yAlert, c8yIdentity, c8yAudits, c8yInventory) {
			var vm = this;
			vm.managerData={}
			vm.ok = function(){
				c8yInventory.create(angular.extend({
					type:"c8y_Manager"
				},vm.managerData)).then(function(res){
					console.log($routeParams);
					//加为子设备
					c8yInventory.addChildAsset($routeParams.groupId,{id:res.data.id}).then(function(res){
						console.log(res);
					...
					//添加身份标志
					c8yIdentity.createIdentity(res.data.id,{type:"c8y_Serial",externalId:vm.managerIdentity}).then(function(res){
						console.log(res);
					})
				})
			}
